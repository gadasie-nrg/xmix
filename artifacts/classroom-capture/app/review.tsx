...
async function preparePhotoForSharing(capture: {
  ...
  if (!capture.uri) throw new Error(t('selectedPhotoUnavailable'));
  if (!FileSystem.cacheDirectory) throw new Error(t('cacheUnavailable'));
  ...
  if (!fileInfo.exists || !fileInfo.size) throw new Error(t('preparePhotoFailed'));
...
async function sharePhotosOnWeb(
  ...
  if (!navigator.share) throw new Error(t('browserSharingUnsupported'));
  ...
  const files = await Promise.all(captures.map(async (capture) => {
    ...
    if (!response.ok) throw new Error(t('browserCouldNotRead'));
    const blob = await response.blob();
    if (!blob.size) throw new Error(t('emptyPhoto'));

    return new File([blob], `xmix-${capture.id}.${extension}`, { type: mimeType });
  }));
  const shareData: ShareData = {
    files,
    title: 'תמונת כיתה מ-XmiX',
    ...(note ? { text: note } : {}),
  };
  if (navigator.canShare && !navigator.canShare(shareData)) {
    throw new Error(t('browserMultiShareUnsupported'));
  }
...
export default function ReviewScreen() {
  ...
  const shareDestinationDescription = Platform.OS === 'web'
    ? 'שתפו את התמונות שנבחרו דרך הדפדפן'
    : canShareMultipleNatively
      ? 'שלחו את כל התמונות שנבחרו יחד'
      : t('shareOneOrAll');
  ...
  const shareToDrive = async (folder: DriveFolder) => {
    ...
    try {
      ...
      setDestination(null);
      Alert.alert(t('sharedSafely'), t('uploadedTo', { count: selectedCaptures.length, value: folder.name }));
    } catch (error) {
      Alert.alert(t('somePhotosNotShared'), error instanceof Error ? error.message : t('tryAgain'));
    } finally {
      setSharing(false);
    }
  };
  ...
  const shareToApp = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        if (!nativeModule && photos.length > 1) {
          throw new Error(t('multipleShareBuild'));
        }
        ...
        if (nativeModule) {
          ...
          await Sharing.shareAsync(shareFiles[0].uri, {
            mimeType: shareFiles[0].mimeType,
      ...
      Alert.alert(
        t('sharingComplete'),
        t('openedInSheet', { count: photos.length }),
      );
    } catch (error) {
      Alert.alert(t('couldNotShare'), error instanceof Error ? error.message : t('tryAgain'));
    } finally {
      setSharing(false);
    }
  };
    ...
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 30 }]} keyboardShouldPersistTaps="handled">
        <View style={styles.header}>
          <Pressable testID="review-back-button" onPress={() => router.back()} style={[styles.backButton, { backgroundColor: colors.card }]}><Feather name="arrow-right" size={20} color={colors.foreground} /></Pressable>
          <View style={styles.headerCopy}><Text style={[styles.headerTitle, { color: colors.foreground }]}>{t('reviewAndShare')}</Text><Text style={[styles.headerMeta, { color: colors.mutedForeground }]}>{t('selected', { count: selectedCaptures.length })}</Text></View>
        ...
        <View style={styles.selectionIntro}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>{t('choosePhotos')}</Text>
          <Text style={[styles.sectionMeta, { color: colors.mutedForeground }]}>{t('choosePhotosDescription')}</Text>
        </View>
        ...
        <View style={styles.captionHeader}><Text style={[styles.sectionTitle, { color: colors.foreground }]}>{t('addNote')}</Text><Text style={[styles.optional, { color: colors.mutedForeground }]}>{t('optional')}</Text></View>
        <TextInput
        ...
        <Pressable
          testID="share-button"
          onPress={() => selectedCaptures.length ? setDestination('apps') : Alert.alert(t('selectPhoto'), t('selectPhotoDescription'))}
          style={({ pressed }) => [styles.shareButton, { backgroundColor: colors.primary, opacity: pressed ? 0.82 : 1 }]}
        >
          <Feather name="send" size={18} color={colors.primaryForeground} />
          <Text style={[styles.shareButtonText, { color: colors.primaryForeground }]}>{t('share')} {selectedCaptures.length ? `${selectedCaptures.length} ${selectedCaptures.length === 1 ? t('photo') : t('photos')}` : t('photos')}</Text>
          <Feather name="arrow-left" size={18} color={colors.primaryForeground} />
        </Pressable>

        <View style={[styles.legend, { borderTopColor: colors.border }]}>
          <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: colors.primary }]} /><Text style={[styles.legendText, { color: colors.mutedForeground }]}>{t('sent')}</Text></View>
          <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: colors.accent }]} /><Text style={[styles.legendText, { color: colors.mutedForeground }]}>{t('notSent')}</Text></View>
          <Text style={[styles.legendHint, { color: colors.mutedForeground }]}>{t('copiesExpire')}</Text>
        </View>
      </ScrollView>
        ...
        <View style={styles.modalBackdrop}>
          <View style={[styles.sheet, { backgroundColor: colors.background, paddingBottom: insets.bottom + 18 }]}>
            <View style={styles.sheetHandle} />
              <View style={styles.sheetHeader}><View><Text style={[styles.sheetTitle, { color: colors.foreground }]}>{t('chooseShareDestination')}</Text><Text style={[styles.sheetMeta, { color: colors.mutedForeground }]}>{caption.trim() ? t('noteIncluded') : t('stayPrivate')}</Text></View><Pressable onPress={() => !sharing && setDestination(null)}><Feather name="x" size={21} color={colors.foreground} /></Pressable></View>
            {destination === 'apps' ? (
               <View style={styles.destinationList}>
                <Pressable testID="drive-destination-button" onPress={openDrive} style={[styles.destination, { backgroundColor: colors.card, borderColor: colors.border }]}>
                  ...
                  <View style={styles.destinationCopy}><Text style={[styles.destinationTitle, { color: colors.foreground }]}>{t('googleDriveFolder')}</Text><Text style={[styles.destinationText, { color: colors.mutedForeground }]}>{t('uploadSelectedWithNote')}</Text></View><Feather name="chevron-left" size={18} color={colors.mutedForeground} />
                </Pressable>
                <Pressable testID="native-share-button" onPress={shareToApp} disabled={sharing} style={[styles.destination, { backgroundColor: colors.card, borderColor: colors.border }]}>
                  ...
                  <View style={styles.destinationCopy}><Text style={[styles.destinationTitle, { color: colors.foreground }]}>{t('anotherApp')}</Text><Text style={[styles.destinationText, { color: colors.mutedForeground }]}>{shareDestinationDescription}</Text></View><Feather name="chevron-left" size={18} color={colors.mutedForeground} />
                </Pressable>
              </View>
            ) : loadingFolders ? (
               <Text style={[styles.loadingText, { color: colors.mutedForeground }]}>{t('loadingDriveFolders')}</Text>
             ) : (
              <ScrollView style={styles.folderList}>
                {folders.map((folder) => <Pressable key={folder.id} testID={`drive-folder-${folder.id}`} onPress={() => shareToDrive(folder)} disabled={sharing} style={[styles.folder, { backgroundColor: colors.card, borderColor: colors.border }]}><View style={[styles.folderIcon, { backgroundColor: folder.name === 'Xmix Capture Temporary' ? colors.accent : colors.secondary }]}><Feather name="folder" size={18} color={folder.name === 'Xmix Capture Temporary' ? colors.accentForeground : colors.primary} /></View><Text style={[styles.folderName, { color: colors.foreground }]}>{folder.name}</Text><Feather name="arrow-up-right" size={16} color={colors.mutedForeground} /></Pressable>)}
              </ScrollView>
            )}
              {sharing ? <Text style={[styles.loadingText, { color: colors.primary }]}>{t('sharingSecurely')}</Text> : null}
          </View>
        </View>
      </Modal>
    </View>
